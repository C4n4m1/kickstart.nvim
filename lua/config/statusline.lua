...
local function diagnostics()
  ...
  local error = ' ' .. num_error
  local warning = '  ' .. num_warning
  local hint = '  ' .. num_hint
  local diag = ''

  local lsp_info = { { num_error, error }, { num_warning, warning }, { num_hint, hint } }
  for _, item in ipairs(lsp_info) do
    if item[1] ~= 0 then
      diag = diag .. ' ' .. item[2]
    end
  end
  return '%#statusline_diagnostics# ' .. diag
end
